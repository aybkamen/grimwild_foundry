			...
			<li class="flexrow items-header stroke stroke-bottom">
				...
				<div class="item-controls">
					...
					<button
						...
						:data-pack="`${game.system.id}.arcana`"
					><i class="fas fa-atlas"></i>Compendium</button>
				</div>
			</li>
			<!-- Arcana rows -->
			<li
				v-for="(item, id) in context.itemTypes.arcana"
				:key="id"
			...
			>
				...
				<div v-if="item.system.description" class="item-description-wrapper">
					<div class="item-description flexcol">
						<em>{{ tierLabel(item.system.tier) }}</em>
						<div v-if="item.system.touchstones" class="item-touchstones">
							<strong>Touchstones:</strong>
							<em>{{ item.system.touchstones }}</em>
						</div>
						<div v-if="item.system.limitations" class="item-limitations">
							<strong>Limitations:</strong>
							<div
								class="item-limitations-description"
								v-html="context.editors[`items.${item.id}.system.limitations`].enriched"
							></div>
						</div>
						<hr />
						<div
							class="item-description"
							v-html="context.editors[`items.${item.id}.system.description`].enriched"
						></div>
						<div v-if="item.system.notes.description" class="item-notes">
							...
							<div
								...
								v-html="context.editors[`items.${item.id}.system.notes.description`].enriched"
							></div>
						</div>
					</div>
				</div>
